...
class UserInput
  ...
  COMMANDS_TEXT = [
    'create station', 'create train','add car', 'remove car', 'create route',
    'edit route', 'set route', 'forward', 'backward', 'list stations',
    'list trains station', 'stop'
  ]
  ...
  def list_stations
    if  @database.stations.length == 0
      puts 'Stations not is'
    else
